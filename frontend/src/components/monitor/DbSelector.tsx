    ...
    <div className="bg-surface border border-border rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5">
        <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 hover:opacity-80">
          ...
          <span className="text-xs font-medium text-text-primary">{selectedConns.length} de {connections.length} bancos</span>
          {selectedConns.length > 0 && !expanded && (
            <span className="text-[10px] text-text-tertiary ml-2 max-w-md truncate">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={clearAll} className="text-[10px] text-text-tertiary hover:text-gray-300">Limpar</button>
        </div>
      </div>
      {expanded && (
        <div className="px-4 pb-3 border-t border-border pt-3">
          <input type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Buscar banco..." className="w-full px-3 py-1.5 mb-2 bg-surface-elevated border border-border rounded-lg text-xs text-text-primary placeholder-gray-400 dark:placeholder-gray-600 focus:border-blue-500 outline-none" />
            ...
            {filtered.map(c => {
              ...
              return (
                <button key={c.id} onClick={() => toggleConn(c.id)}
                  className={`px-2 py-1 rounded text-[10px] truncate border transition ${sel ? 'bg-blue-100 dark:bg-blue-600/20 border-blue-400 dark:border-blue-500 text-blue-700 dark:text-blue-300' : 'bg-surface-elevated border-border text-gray-600 dark:text-gray-500 hover:border-gray-400 dark:hover:border-gray-600'}`}>
                  {c.name.replace('SQL / ', '')}
                </button>
              )
            })}
