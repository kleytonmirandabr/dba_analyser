...
export default function ExplorerPage() {
  ...
  const loadSchemas = async (connId: string) => {
    ...
    try {
      ...
      const schemas: TreeNode[] = data.data.map((s: string) => ({
        ...
        children: [
          { id: `${s}:tables`, label: 'Tabelas', type: 'table' as NodeType, schema: s, children: [], loaded: false },
          { id: `${s}:views`, label: 'Views', type: 'view' as NodeType, schema: s, children: [], loaded: false },
          { id: `${s}:functions`, label: 'Funções', type: 'function' as NodeType, schema: s, children: [], loaded: false },
  ...
  const renderNode = (node: TreeNode, depth = 0): JSX.Element => {
    ...
    return (
      <div key={node.id}>
        <div
          className={`flex items-center gap-1.5 px-2 py-1 hover:bg-gray-800 rounded cursor-pointer text-sm ${detail && detailType.includes(node.label) ? 'bg-gray-800 text-blue-400' : 'text-gray-300'}`}
        ...
        >
          {hasChildren ? (isExpanded ? <ChevronDown className="w-3 h-3 text-gray-500" /> : <ChevronRight className="w-3 h-3 text-gray-500" />) : <span className="w-3" />}
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="w-80 bg-gray-900 border-r border-gray-800 flex flex-col">
        ...
        <div className="p-3 border-b border-gray-800">
          <select className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-white"
            value={selectedConn} onChange={e => loadSchemas(e.target.value)}>
        ...
        <div className="p-3 border-b border-gray-800">
          <div className="relative">
            ...
            <input className="w-full pl-8 pr-3 py-1.5 bg-gray-800 border border-gray-700 rounded text-xs text-white"
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {loading ? <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-blue-500" /></div>
            ...
            : <p className="text-xs text-gray-600 text-center py-8">Selecione uma conexão para explorar</p>}
        </div>
      </div>
      ...
      <div className="flex-1 bg-gray-950 p-6 overflow-auto">
        {detail ? (
          <div>
            <h2 className="text-lg font-bold text-white mb-4">{detailType}</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-800">
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">Coluna</th>
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">Tipo</th>
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">Null</th>
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">Default</th>
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">PK</th>
                    <th className="text-left py-2 px-3 text-gray-400 font-medium">FK</th>
                  </tr>
                </thead>
                <tbody>
                  {detail.map((col: any, i: number) => (
                    <tr key={i} className="border-b border-gray-900 hover:bg-gray-900/50">
                      ...
                      <td className="py-2 px-3 text-gray-500 font-mono text-xs">{col.defaultValue || '-'}</td>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-gray-600">Selecione um objeto na árvore para ver detalhes</p>
            </div>
          </div>
        )}
      </div>
