    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-text-primary flex items-center gap-2">
          <Stethoscope className="w-6 h-6 text-emerald-400" /> {t('diagnostics.title')}
        </h1>
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2.5">
          <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 hover:opacity-80">
            ...
            <span className="text-xs font-medium text-text-primary">{selectedConns.length} de {connections.length} bancos selecionados</span>
          </button>
        ...
        {expanded && (
          <div className="px-4 pb-3 border-t border-border pt-3">
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Buscar banco..." className="w-full px-3 py-1.5 mb-2 bg-surface-elevated border border-border rounded-lg text-xs text-text-primary placeholder-gray-600 focus:border-emerald-500 outline-none" />
              ...
              {filtered.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => toggleConn(c.id)}
                    className={`px-2 py-1 rounded text-[10px] truncate border transition relative ${sel ? 'bg-emerald-600/20 border-emerald-500 text-emerald-300' : 'bg-surface-elevated border-border text-text-tertiary hover:border-gray-600'}`}>
      ...
      {selectedConns.length === 0 && (
        <div className="text-center py-16 text-text-tertiary">
          ...
          <p className="text-sm">{t('diagnostics.selectAndAnalyze')}</p>
        </div>
      )}
      ...
      {selectedConns.map(connId => {
        ...
        return (
          <div key={connId} className="bg-surface border border-border rounded-xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center gap-3">
                <Database className="w-4 h-4 text-blue-400" />
                <span className="text-sm font-medium text-text-primary">{conn.name}</span>
                <span className="text-[10px] text-text-tertiary">{conn.dbType}</span>
              </div>
              <div className="flex items-center gap-3">
                {report && (
            ...
            {isLoading && (
              <div className="p-8 flex items-center justify-center">
                ...
                <span className="ml-2 text-sm text-text-secondary">Coletando métricas e analisando...</span>
              </div>
            )}
            ...
            {report && !isLoading && (
              <div className="p-4 space-y-3">
                ...
                {report.diagnostics.length === 0 ? (
                  <div className="p-4 text-center text-green-400 text-sm">{t('diagnostics.noIssues')}</div>
                ) : (
                ...
                {report.metrics.missingIndexDetails && (
                  <div className="mt-3 p-3 bg-gray-100/50 dark:bg-gray-800/50 rounded-lg border border-border">
                    <p className="text-[10px] text-text-tertiary uppercase mb-2 flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Top índices sugeridos</p>
                    <div className="space-y-1">
                      {report.metrics.missingIndexDetails.map((idx: any, i: number) => (
                        <div key={i} className="flex items-center justify-between text-[11px]">
                          <span className="text-text-secondary truncate flex-1">{idx.table}</span>
...
function MetricChip({ label, value, warn, crit }: { label: string; value: string; warn?: boolean; crit?: boolean }) {
  const color = crit ? 'text-red-400 border-red-800/50' : warn ? 'text-yellow-400 border-yellow-800/50' : 'text-text-secondary border-border'
  return (
    <div className={`p-2 rounded border bg-gray-100/30 dark:bg-gray-800/30 ${color}`}>
      <p className="text-[9px] text-text-tertiary">{label}</p>
      <p className={`text-xs font-bold ${crit ? 'text-red-400' : warn ? 'text-yellow-400' : 'text-text-primary'}`}>{value}</p>
    </div>
  )
}
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-[10px] text-text-tertiary uppercase font-medium">{d.category}</span>
            <span className="text-[9px] text-gray-600">Prioridade {index + 1}</span>
          </div>
          <p className="text-xs text-text-primary font-medium mb-1">{d.symptom}</p>
          <p className="text-[11px] text-text-secondary mb-1"><strong className="text-text-secondary">Causa:</strong> {d.cause}</p>
            ...
            <div className="mt-2">
              ...
              {showSql && (
                <div className="mt-1 relative">
                  <pre className="p-2 bg-surface rounded text-[10px] text-text-secondary overflow-x-auto font-mono">{d.sql}</pre>
                  <button onClick={copy} className="absolute top-1 right-1 p-1 bg-surface-elevated rounded hover:bg-surface-active">
